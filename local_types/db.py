...
class base():
    ...
    def getConnString(self, filename="/home/u/idapro-7.7/plugins/local_types/localTypes.toml", section="database", param="connection_info") -> dict:
        parser = ConfigParser()
    ...
    def insertIntoDB(self, type_name: str, typedef: str, file_path: str) -> None:    
        ...
        try:            
            ...
            conn = connect(**params)                
            cur = conn.cursor()	
            req = "INSERT INTO types (file_path, typedef, name) VALUES ('" + file_path +"', '" + typedef + "', '" + type_name + "');"
            print(req)
            cur.execute(req)            
            conn.commit()            
            cur.close()
